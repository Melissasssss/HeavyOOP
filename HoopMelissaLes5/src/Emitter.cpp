...
void Emitter::start()
{
	...
	for (int i = 0; i < MAXPARTICLES; i++) {
		...
		particles1.push_back(part1);
		part1.init(locationEmi);
		//setRandomColor(part1[i]);
	}

	for (int i = 0; i < MAXPARTICLES; i++) {
		CurveParticle part2 = CurveParticle();
		particles2.push_back(part2);
		part2.init(locationEmi);
...
void Emitter::update()
{
	...
	vector<Particle>::iterator i = particles1.begin();
	while (i != particles1.end()) {
		i->move();
		i++;
	}
